...
def Verificar_Atualizacoes():
    try:
        ...
        versao = requests.get(
            "https://api.github.com/repos/Saralapa/League-Lobby-Clicker/releases/latest"
        )
        ...
        if versao["tag_name"].replace("v", "") != versao_atual:
            if idioma.lower() == "portugues":
                ...
                resposta = messagebox.askquestion(
                    "Update Available",
                    f"Current version: {versao_atual}\nLatest Version: {versao['tag_name'].replace("v","")}\n\n\nDownload now?",
                )
            if resposta == "yes":
                webbrowser.open(
                    "https://github.com/Saralapa/League-Lobby-Clicker/releases/latest"
                )
        return resposta
    except:
        None
