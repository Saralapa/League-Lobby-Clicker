...
class BotoesBarraDeTitulo:
    def BotaoFechar():
        ...
        botao_fechar_pressionado = False
        botao_fechar = tk.Label(
            frame_barra_de_titulo,
            text="X",
            font=("Arial", 14),
            bg=frame_barra_de_titulo.cget("background"),
            fg=cor,
            relief="flat",
        )
    ...
    def BotaoMinimizar():
        ...
        def BotaoMinimizarSolto():
            ...
            if mouse_sobre_botao_minimizar:
                [
                    window
                    for window in gw.getWindowsWithTitle(root.title())
        ...
        botao_minimizar.bind(
            "<ButtonPress-1>", lambda event: BotaoMinimizarPressionado()
        )
    ...
    def BotaoDoacao():
        ...
        botao_doacao_pressionado = False
        botao_doacao = tk.Label(
            frame_barra_de_titulo,
            image=imagem_doacao,
            bg=frame_barra_de_titulo.cget("background"),
        )
        botao_doacao.place(
            width=45, height=frame_barra_de_titulo.winfo_reqheight(), x=0, y=1
        )
...
def MoverJanela():
    ...
    def AlterarPosicaoJanela(event):
        global posicao_clique_barra_de_titulo_x, posicao_clique_barra_de_titulo_y
        nova_posicao_x = root.winfo_x() + (
            event.x_root - posicao_clique_barra_de_titulo_x
        )
        nova_posicao_y = root.winfo_y() + (
            event.y_root - posicao_clique_barra_de_titulo_y
        )
...
def Criar_Barra_de_Titulo(icone):
    ...
    label_icone = tk.Label(
        frame_barra_de_titulo, image=icone, bg=frame_barra_de_titulo.cget("background")
    )
