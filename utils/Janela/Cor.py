...
def Alterar_Cor(image, cor_original, cor_nova):
    ...
    for i, pixel in enumerate(pixel_data):
        if (pixel[0] > cor_original[0]-tolerancia or pixel[0] < cor_original[0]+tolerancia) and (pixel[1] > cor_original[1]-tolerancia or pixel[1] < cor_original[1]+tolerancia) and (pixel[2] > cor_original[2]-tolerancia or pixel[2] < cor_original[2]+tolerancia):
...
def Atualizar_Cor(valor):
    from utils.Janela.Janela import cor, frame_botao_desfazer, botao_desfazer, frame_botao_confirmar, botao_confirmar, label_borda_topo, label_borda_inferior, \
    frame_botoes_idiomas, botoes_idiomas, frame_botoes_modos_de_jogo, botoes_modos_de_jogo, frame_botoes_roles, botoes_roles, label_auto_aceitar, frame_botao_cor_personalizada, \
    botao_cor_personalizada, label_titulo, botao_minimizar, botao_fechar, imagem_cor, imagem_idioma, imagem_doacao, botao_icone_cor, botao_icone_idioma, botao_doacao
    ...
    if valor != None:
        ...
        return cor, imagem_cor, botao_icone_cor, imagem_idioma, botao_icone_idioma, imagem_doacao, botao_doacao
...
def CriarBotoesCores(imagem_cor):
    ...
    botao_cor_personalizada = tk.Button(frame_botao_cor_personalizada, text="Cor personalizada", command=lambda: func_Cor(colorchooser.askcolor()[1]), bg="#1f1f1f", fg=cor)
